/**
 * Multi-Sheet Setup Script
 *
 * Creates tabs with headers in multiple Google Spreadsheets (one per module).
 * Fill in the spreadsheet IDs below after creating them in Google Sheets.
 *
 * Usage: npx tsx scripts/setup-multi-sheets.ts
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// FILL THESE IN after creating spreadsheets in Google Sheets
// ─────────────────────────────────────────────────────────────────────────────

const SPREADSHEET_IDS: Record<string, string> = {
  "01_master_data":   process.env.SHEET_ID_MASTER_DATA   || "",
  "03_planning":      process.env.SHEET_ID_PLANNING      || "",
  "04_supply_chain":  process.env.SHEET_ID_SUPPLY_CHAIN  || "",
  "05_site_ops":      process.env.SHEET_ID_SITE_OPS      || "",
  "06_finance":       process.env.SHEET_ID_FINANCE       || "",
  "07_dashboards":    process.env.SHEET_ID_DASHBOARDS    || "",
};

// ─────────────────────────────────────────────────────────────────────────────
// Sheet definitions per module
// ─────────────────────────────────────────────────────────────────────────────
...
const MODULES: Record<string, SheetDef[]> = {
  "01_master_data": [
...
async function main() {
  ...
  for (const [moduleKey, sheetDefs] of Object.entries(MODULES)) {
    const spreadsheetId = SPREADSHEET_IDS[moduleKey];
    if (!spreadsheetId) {
      console.log(`\n⏭ Skipping ${moduleKey} — no spreadsheet ID configured`);
      continue;
    }

    console.log(`\n📋 Setting up ${moduleKey} (${spreadsheetId.substring(0, 12)}...)`);

    // Create tabs
    const requests = sheetDefs.map((sheet, index) => ({
      addSheet: {
        properties: {
          title: sheet.name,
          index,
    ...
    try {
      ...
      console.log(`  Created ${sheetDefs.length} tabs`);
    } catch (error) {
      console.log(`  Tabs may already exist, adding headers...`);
    }
    ...
    for (const sheet of sheetDefs) {
      try {
        ...
        console.log(`  ✗ ${sheet.name} — ${(error as Error).message}`);
      }
    }
  }
